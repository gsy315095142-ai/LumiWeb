...
function simulateUserScan() {
  ...
  document.getElementById('exchangeScanStep').innerHTML = '<div style="text-align:center;padding:12px 0;"><div style="background:rgba(255,255,255,0.04);border-radius:12px;padding:14px;margin-bottom:10px;border:1px solid var(--accent2);"><div style="font-size:1.5em;">' + exScannedProduct.icon + '</div><div style="color:#fff;font-weight:600;font-size:0.85em;">' + exScannedProduct.name + '</div><div style="margin-top:4px;"><span style="color:var(--accent2);font-weight:700;">' + exScannedProduct.price + ' 💎</span><span style="text-decoration:line-through;color:var(--muted);font-size:0.65em;margin-left:4px;">￥' + exScannedProduct.origPrice + '</span></div></div><div class="divider"></div><div style="color:#ccc;font-size:0.85em;"><strong>' + exScannedUser.name + '</strong></div><div style="color:var(--muted);font-size:0.7em;">💰 预测币: ' + exScannedUser.gameCoin + ' | 💎 礼品点数: ' + exScannedUser.exCoin + '</div><div class="divider"></div><button class="btn btn-purple btn-block" onclick="confirmExchange2()">✅ 确认核销 · 扣除 ' + exScannedProduct.price + ' 💎</button></div>';
}
