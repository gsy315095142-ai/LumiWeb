...
管理员扫码 → 发币（预测币到账提示）→ 录入选手信息 → 归入选手池
    ↓
<span class="highlight">【主动报名】或【随机匹配】（二次确认）</span>
    ↓
┌─────────────────────┬─────────────────────┐
│     主动报名         │      随机匹配        │
│  已报名 xx 人       │   名称跳动 ~5 秒     │
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          '报名阶段支持 <strong class="text-primary">主动报名</strong> 与 <strong class="text-primary">随机匹配</strong> 两种模式',
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ['按钮', '行为概要'],
        rows: [
          ['<span class="btn-pill">主动报名</span>', '开放报名，展示已报名人数与名单；管理员手动确认红/蓝方'],
          ['<span class="btn-pill">随机匹配</span>', '从选手池自动抽取双方选手，展示匹配动画；支持单方重抽'],
        ],
      },
      {
        type: 'rules',
        items: [
          '<strong class="text-primary">主动报名</strong> 与 <strong class="text-primary">随机匹配</strong> <strong class="text-accent">互斥</strong>，同一局只能选择其中一种',
          '进入任一模式后，管理员可 <strong class="text-primary">返回上一步</strong>，重新选择报名方式',
        ],
      },
    ],
    mockup: 'entry',
    ...
    blocks: [
      { type: 'subsection', title: '6.1 匹配过程展示' },
      {
        ...
        items: [
          {
            id: '#6',
            text: '点击 <span class="btn-pill">随机匹配</span> 后，投屏匹配页与小程序对应 <strong>选手名称位置</strong>，出现 <strong>选手名称跳动</strong> 效果。',
          },
          {
            id: '6.1',
      ...
      { type: 'subsection', title: '6.2 匹配算法' },
      {
        ...
        items: [
          ...
          { id: '6.5', text: '选手池 <strong>人数不足</strong> 时，点击 <span class="btn-pill">随机匹配</span> 后弹出提示：<strong>「人数不足，无法随机匹配」</strong>' },
        ],
      },
        ...
        <div class="mode-split">
          <div class="mode-card manual">
            ...
            <ul>
              ...
              <li>管理员确认红/蓝方</li><li>切换为选手名称展示</li>
            </ul>
          </div>
          <div class="mode-card random">
            <h4>随机匹配</h4>
            <ul>
              <li>名称跳动约 5 秒</li><li>去最高/最低分后抽人</li>
              <li>积分接近者中随机对手</li><li>支持单方重新匹配</li>
            </ul>
          </div>
        </div>`;
    case 'mockup':
