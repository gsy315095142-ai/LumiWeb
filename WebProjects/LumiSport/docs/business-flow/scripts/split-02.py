#!/usr/bin/env python3
"""Split 02 HTML into partials and rebuild the page."""
from pathlib import Path
import re

ROOT = Path(__file__).resolve().parent.parent
HTML_FILE = ROOT / "02-选手报名与观众竞猜流程.html"
PARTIALS = ROOT / "partials" / "02"
ASSETS = ROOT / "assets"


def extract_svg(html: str) -> str:
    match = re.search(r"<svg class=\"qr-grid\"[^>]*>.*?</svg>", html, re.S)
    if not match:
        raise ValueError("QR SVG not found")
    return match.group(0).replace('class="qr-grid" ', "")
...
def main() -> None:
    ...
    (ASSETS / "qr-placeholder.svg").write_text(svg, encoding="utf-8")
    ...
    print(f"Wrote {ASSETS / 'qr-placeholder.svg'}")
    for name in parts:
