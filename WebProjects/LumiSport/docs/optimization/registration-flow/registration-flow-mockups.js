...
const MOCKUPS = {
    ...
    const adminBody = `
      ...
      <div class="mk-modal-overlay">
        <div class="mk-modal">
          ...
          <div class="mk-modal-text">开放后投屏与小程序将展示已报名人数；期间仍可使用随机匹配按钮。</div>
  ...
  manual() {
    ...
    const adminBody = `
      ${mkHero('第 3 局 · 报名中 · 疾速冰球', '<span class="mk-reg-badge">已报名：5 人</span>', '<span class="mk-reg-badge">已报名：5 人</span>')}
      <div class="mk-card">
        <div class="mk-card-title">报名进行中 · 随机匹配</div>
        <p class="mk-hint">仍从选手池按随机匹配算法抽取</p>
        <button class="mk-btn mk-btn-purple">🎲 随机匹配</button>
        <div class="mk-btn-row">
          <button class="mk-btn mk-btn-orange">🔵 蓝方随机匹配</button>
          <button class="mk-btn mk-btn-orange">🔴 红方随机匹配</button>
        </div>
      </div>`;
    ...
    const screenBody = `
      ...
      ${mkHero('等待管理员确认或随机匹配', '<span class="mk-reg-badge">已报名：5 人</span>', '<span class="mk-reg-badge">已报名：5 人</span>')}`;

    return mkWrapStacked('界面示意 · 主动报名（名单 + 随机匹配）',
